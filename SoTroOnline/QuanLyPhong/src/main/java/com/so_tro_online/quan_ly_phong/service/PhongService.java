...
public class PhongService implements IPhongService{
    ...
    @Override
    public PagedResponse<RoomResponse> getAllRoomsPaged(int page, int size) {
    ...
    @Override
    public PagedResponse<RoomResponse> getAllRoomsActivePaged(int page, int size) {
        Pageable pageable = PageRequest.of(page, size);
    ...
    public RoomResponse createRoom(RoomRequest roomRequest) {
        ...
        if(phongRepository.existsByTenPhongAndTrangThaiNot(roomRequest.getTenPhong(),TrangThai.daXoa)){
    ...
    public RoomResponse updateRoom(Integer id, RoomRequest roomRequest) {
        Phong phong= phongRepository.findByMaPhongAndTrangThaiNot(id,TrangThai.daXoa)
                .orElseThrow(()->new ReseourceNotFoundException("không tìm thấy phòng với id: "+id));
        // Check for room name duplicates among all non-deleted rooms except the current one
        if(phongRepository.existsByTenPhongAndMaPhongNotAndTrangThaiNot(roomRequest.getTenPhong(),id,TrangThai.daXoa)){
            throw new RoomAlreadyExist("phòng đã tồn tại: "+roomRequest.getTenPhong());
        }
        TaiKhoan taiKhoan=taiKhoanRepository.findByMaTaiKhoanAndTrangThai(roomRequest.getMaQuanLy(), com.so_tro_online.quan_ly_tai_khoan.entity.TrangThai.hoatDong)
        ...
        try (InputStream is = file.getInputStream();
            ...
            for (int i = 1; i <= sheet.getLastRowNum(); i++) { // bỏ header (row 0)
                ...
                try {
                    ...
                    phong.setTaiKhoan(taiKhoan);
                    if(phongRepository.existsByTenPhongAndTrangThaiNot(row.getCell(1).getStringCellValue(),TrangThai.daXoa)){
                        throw new RoomAlreadyExist("phòng đã tồn tại: "+row.getCell(1).getStringCellValue());
                    }
