...
import com.so_tro_online.utils.IdGenerator;
...
public class KhachThueService {
    ...
    public KhachThueDto createKhachThue(KhachThueRequest khachThueRequest) {
        try {
            ...
            int newMaKhachDaiDien = (int) generateUniqueMaKhachDaiDien();
     ...
     */
    private int generateUniqueMaKhachDaiDien() {
        int maxRetries = 10; // tránh vòng lặp vô hạn
        int attempts = 0;

        while (attempts < maxRetries) {
            int maKhachDaiDien = generateRandomInt(6);
        ...
        // Nếu vẫn trùng sau nhiều lần thử, fallback về timestamp
        int fallbackId = (int) (System.currentTimeMillis() % 1_000_000);
        return fallbackId;
    }

    /**
     * Sinh số nguyên ngẫu nhiên có đúng `length` chữ số.
     * Ví dụ length=6 → trả về giá trị từ 100000 đến 999999.
     */
    ...
    public Optional<KhachThue> getActiveKhachThueById(int maKhach) {
        try {
            Optional<KhachThue> khachThue = khachThueRepository.findByMaKhachDaiDienAndTrangThai(maKhach, TrangThai.hoatDong);
            if (khachThue.isEmpty()) {
                throw new KhachThueNotFoundException(maKhach);
            }
            return khachThue;
