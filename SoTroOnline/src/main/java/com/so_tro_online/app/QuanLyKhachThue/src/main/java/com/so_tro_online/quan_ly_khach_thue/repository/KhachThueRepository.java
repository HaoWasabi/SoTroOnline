...
public interface KhachThueRepository extends JpaRepository<KhachThue, Integer> {
    ...
    boolean existsByMaKhachDaiDien(int maKhachDaiDien);

    Optional<KhachThue> findByMaKhachDaiDienAndTrangThai(int maKhachDaiDien, TrangThai trangThai);

    // Enhanced search methods - exclude deleted tenants
    @Query("""
    SELECT k FROM KhachThue k
    WHERE k.trangThai != :excludedStatus
      AND (
        LOWER(k.hoTen) LIKE LOWER(CONCAT('%', :searchTerm, '%'))
        OR k.maCanCuoc LIKE CONCAT('%', :searchTerm, '%')
        OR CAST(k.maKhachDaiDien AS string) LIKE CONCAT('%', :searchTerm, '%')
        OR CAST(k.maKhach AS string) LIKE CONCAT('%', :searchTerm, '%')
      )
""")
